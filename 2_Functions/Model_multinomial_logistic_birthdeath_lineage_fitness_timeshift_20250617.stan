...
// To add timeshift:
// To add timeshift add betas before and after the timeshift, 
// and then add the betas to the computetheta funcitons 
// Easiest way: loop after year x use 1 beta and then use the other
// Then compare the scores of the model fits
// Potentially compare many timepoints along ~0.5 year intervals 
// Additonal thought: Add change of fitness for some only lineages

// Model design: 
// model lineage dynamics over time, where each lineage (or group of lineages) can:
// arise from an ancestral group,
// grow or decline with its own fitness (growth rate),
// and split into descendant lineages with shared proportions (gamma),
// while observing time-varying counts of each lineage (Y) through a multinomial framework.

// Conceptually:
// How different viral lineages change in relative abundance over time?
// To find out we ask: what is the log-relative fitness values (theta) for each lineage at each timepoint?
// We also account for lineage emergence and splits from parents, then use a softmax over theta to get the proportions
// and observe the lineages as mutinominal draw 





//Fucntions: Utility functions for lineage dynamics (lineage appearance, splits, softmax proportion calc, etc.)
  ...
  real multinomial_zeros_lpmf(array[] int y, vector p){
    ...
    if(n_zeros_in_y > 0){
      ...
      for (i in 1:n_zeros_in_y){
        res = res + binomial_lpmf(0 | sum(y), p[idx[i]]); // Likelihood of a category with 0 count
      }
    }
...
// Data: Includes observed lineage counts Y, times t, lineage tree structure (parents, t_start)
...
// Transf params: Computes theta = log-abundance over time, based on model
transformed parameters { 
  ...
  // The fitness slope of the lineage (beta[k]) is it getting more or less fit
  // or whether it's newly emerging child lineage (governed by gamma splits from parents)
  
  // Ancestral lineages start with an initial frequency from alpha and derived lineages get a 
  // a fraction (gamma) of their parent’s frequency, and change with their own beta (fitness slope)
}
...
// Ouputs: Computes log-likelihoods and smoother theta_new values for visualization
