...
association_scores_per_group = function(dataset_with_nodes, dataset_with_inferred_reconstruction, tree, 
  ...
  for(j in 1:(length(group_names)-1)){ 
    ...
    if(length(ancest) == 0) ancest = Ancestors(x = tree, node = mrca_node, type = 'all')[1]
    ...
    for(i in 1:length(possible_snps)){
      ...
      while(str_detect(ancestral_state, pattern = 'n|X') == TRUE & k <= length(ancest)-1){
        ...
        if (length(idx) == 0) {
          cat("⚠️ No match for ancestor node in branches[,1]\n")
          cat("  Current group j =", j, "\n")
          cat("  Ancestor node (rev(ancest)[", k, "]) =", current_ancestor, "\n")
          cat("  Ancest vector:\n"); print(ancest)
          cat("  Branches head:\n"); print(head(branches))
          break
          }
...
# DENV4 doesn't work and gives error 
# Error in if (nd == rootnd) break : missing value where TRUE/FALSE needed

scores_all[["Dengue_1_E_AA"]]
scores_all[["Dengue_3_2Kfrag_AA"]]
...
########################################################################################################################################
genes <- read.csv("1_Data/1_5_DENV/Position_genes_Dengue.csv")
