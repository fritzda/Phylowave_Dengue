...
association_scores_per_group = function(dataset_with_nodes, dataset_with_inferred_reconstruction, tree, 
  ...
  for(j in 1:(length(group_names)-1)){ 
    ...
    if (is.null(mrca_node) || is.na(mrca_node)) {
      cat("⚠️ getMRCA failed for group", j, "in",dataset_with_nodes, "\n")
      next
      }
    ...
    if (length(ancest) == 0) {
      cat("⚠️ Group", j, "in", virus, gene, level, "has MRCA at root → no ancestors available\n")
    ...
    cat("👉 DEBUG: group", j, "(", group_names[j], ") in", virus, gene, level, "\n")
    ...
    for (i in 1:length(possible_snps)) {
      ...
      if (length(ancest) == 0) {
        ...
        while (str_detect(ancestral_state, pattern = 'n|X') == TRUE & k <= length(ancest) - 1) {
          ...
          if (length(idx) == 0) {
            cat("⚠️ No match for ancestor node in branches[,1] at SNP", i, "\n")
            break
          }
...
####Hunting for node clades to color:
ie: inferring internal node clades based on descendant tips

```{r, eval = FALSE}
dataset_with_nodes_3$clade_inferred <- dataset_with_nodes_3$clade



for (node in (nroot_3):(length(tree_DENV3$tip.label) + tree_DENV3$Nnode)) {
  
  
  
  # Get descendant tip indices for this node
  desc_tips <- unlist(Descendants(tree_DENV3, node, type = "tips"))
  
  # Get clades of those descendant tips
  clade_vals <- dataset_with_nodes_3$clade[desc_tips]
  
  # If all non-NA clades are the same, assign it to this node
  if (length(unique(clade_vals[!is.na(clade_vals)])) == 1) {
    dataset_with_nodes_3$clade_inferred[node] <- unique(clade_vals[!is.na(clade_vals)])
  } else {
    dataset_with_nodes_3$clade_inferred[node] <- NA
  }
}

###DEVN3
## Color key for Nextstrain clades
colors_clade_3 = met.brewer(name="Cross", n=length(levels(as.factor(dataset_with_nodes_3$clade_inferred))), type="continuous")

## Color of each node, based on the key
dataset_with_nodes_3$clade_color = as.factor(dataset_with_nodes_3$clade_inferred)
...
dataset_with_nodes_3$clade_color[is.na(dataset_with_nodes_3$clade_color)] <- "grey"
...
###DENV2
## Color key for Nextstrain clades
colors_clade_2 = met.brewer(name="Cross", n=length(levels(as.factor(dataset_with_nodes_2$clade_inferred))), type="continuous")

## Color of each node, based on the key
dataset_with_nodes_2$clade_color = as.factor(dataset_with_nodes_2$clade_inferred)
...
dataset_with_nodes_2$clade_color[is.na(dataset_with_nodes_2$clade_color)] <- "grey"

###DENV1
## Color key for Nextstrain clades
colors_clade_1 = met.brewer(name="Cross", n=length(levels(as.factor(dataset_with_nodes_1$clade_inferred))), type="continuous")

## Color of each node, based on the key
dataset_with_nodes_1$clade_color = as.factor(dataset_with_nodes_1$clade_inferred)
...
dataset_with_nodes_1$clade_color[is.na(dataset_with_nodes_1$clade_color)] <- "grey"


###DENV4
## Color key for Nextstrain clades
colors_clade_4 = met.brewer(name="Cross", n=length(levels(as.factor(dataset_with_nodes_4$clade_inferred))), type="continuous")

## Color of each node, based on the key
dataset_with_nodes_4$clade_color = as.factor(dataset_with_nodes_4$clade_inferred)
...
dataset_with_nodes_4$clade_color[is.na(dataset_with_nodes_4$clade_color)] <- "grey"
...
# Retrieve relevant variables
  ...
  png(filename = paste0("3_Output_Figures/","Classical_Clades_panel_plot_DENV_", i, ".png"), width = 1800, height = 1800, res = 300)
  par(mfrow = c(2, 1), oma = c(0, 0, 0, 0), mar = c(4, 4, 0, 0))
...
for (i in 1:4) {
  ...
  svglite(filename = paste0("3_Output_Figures/", "DENV", i, "New_Clades.svg" ), width = 8, height = 10)
...
for (i in 1:4) {
  ...
  ggsave(filename = paste0("3_Output_Figures/","DENV_", i, "_", "Classical_vs_Phylowave_Clades_combined_plot_", st, ".png"), plot = combined_plot, width = 12, height = 6, dpi = 300)
  ...
  htmlwidgets::saveWidget(interactive_plot, file = paste0("3_Output_Figures/","DENV_", i, "Classical_vs_Phylowave_Clades_interactive_plot_", st,".html"))
}
...
options(mc.cores = 14) #Set to 14 coresMore actions
start_time = Sys.time()
...
#### 21 How are LOO and WAIC related?
...
Vehtari, Gelman and Gabry (2017) show that PSIS-LOO has usually smaller error in estimating ELPD than WAIC. The exception is the case when p_loo ≪𝑁
, as then WAIC tends to have slightly smaller error, but in that case both PSIS-LOO and WAIC have very small error and it doesn’t matter which computational approximation is used. On the other hand, for flexible models WAIC fails more easily, has significant bias and is less easy to diagnose for failures. WAIC has been included in loo package only for comparison purposes and to make it easy to replicate the results by Vehtari, Gelman and Gabry (2017).
...
The log probabilities are easier. For example, Guido Biele had a problem computing elpd_loo with a beta-binomial model for data with 22 categories. Computed individual elpd_loo values for observations were around -461. For discrete model with uniform probability for 22 categories log probabilities would be log(1/22)≈−3.1
, and thus there was two orders of magnitude error in log scale. With the fixed code individual elpd_loo values were about −2.3>−3.1
, that is, the model was beating the uniform distribution.
...
As quick rule: If elpd difference (elpd_diff in loo package) is less than 4, the difference is small (Sivula, Magnusson and Vehtari, 2020, p. McLatchie+etal:2023). If elpd difference (elpd_diff in loo package) is larger than 4, then compare that difference to standard error of elpd_diff (provided e.g. by loo package) (Sivula, Magnusson and Vehtari, 2020). The value for deciding what is small or large can be based on connection to Pseudo-BMA+-weights (Yao et al., 2018, p. McLatchie+etal:2023). See also How to interpret in Standard error (SE) of elpd difference (elpd_diff)? and Case studies for the paper Uncertainty in Bayesian LOO-CV Model Comparison
...
for (i in seq_along(serotypes)) {
  ...
  for (bp in breakpoints) {
    cat("  └── Fitting ", serotype_name, " breakpoint model at year:", bp, "\n")
...
# >1.96 → ~95% confidence one model is better than the other.
...
# — Vehtari, Gelman, Gabry (2017), "Practical Bayesian model evaluation using leave-one-out cross-validation and WAIC" (arXiv:1507.04544)
...
for (i in 1:4) {
  ...
  png(filename = paste0("3_Output_Figures/fitness_plot_DENV_", i, "_" , st, "_min_year_", min_year_model, ".png"), width = 1600, height = 1400, res = 300)
...
-   *downstream_window*: for each group, consider nodes from the MRCA up
    to this time. Sometimes there are heterogeneous groups that aquire mutations way later (not lineage causing, but still would be lineage defining)
...
#     # Extract virus type and gene
#     parts <- strsplit(recon_name, "_", fixed = TRUE)[[1]]
#     virus <- paste(parts[1], parts[2], sep = "_")
#     gene <- paste(parts[3:length(parts)], collapse = "_")
# 
#     cat("For:", virus, ",level", level, "\n")
#     
#     result <- tryCatch({
#       association_scores_per_group(
#         dataset_with_nodes = dataset_with_nodes_all[[virus]],
#         dataset_with_inferred_reconstruction = recon_obj[[paste0("dataset_with_inferred_reconstruction_", level)]],
#         tree = tree_DENV_all[[virus]],
#         possible_snps = recon_obj$possible_snps,
...
#       )
#     }, error = function(e) {
#       cat("⚠️ ERROR for:", virus, gene, "level:", level, "\n")
#       cat("Message:", e$message, "\n")
#       return(NULL)
#     })
# 
#     scores_all[[paste0(virus, "_", gene, "_", level)]] <- result
#   }
# }

# rm(virus,tree,nodes,recon,group_names,group_tips)

#DENV4 Group 2 is at the MRCA (no ancestors) so changed the association code to permit this.


scores_all[["Dengue_4_E_AA"]]
scores_all[["Dengue_4_frag2k_codon"]]

saveRDS(scores_all, file= here("4_Output_Data", "4_5_DENV", paste0("scores_all_uw_", upstream_window,"_dw_", downstream_window, "_", st,".RData")))
...
OLD Version to do manaully 
```{r, eval = FALSE}


## Denv1
scores_DENV1_thai_codon = association_scores_per_group(dataset_with_nodes = dataset_with_nodes_1,
                                                     dataset_with_inferred_reconstruction = reconstructions$DENV1_E$dataset_with_inferred_reconstruction_codon, 
                                                     tree = tree_DENV1_thai, 
                                                     possible_snps = reconstructions$DENV1_E$, 
                                                     upstream_window = upstream_window, 
                                                     downstream_window = downstream_window)
...
When doing reconstruciton, can set some nodes in the branch that have high levels of support to NA. Since sig_threshold is based on the nodes, this is a limitaiton. Have to have non-NA on the branch tip 

Noise in sig snp determination: 
* phylogeny reconstruction: possible that some members don't have the snp 

Lineage defining is:
* present in 80% of groups and not present in parent nodes.
* 
...
## Find significant snps
########################################################################################################################################
sig_threshold <- 0.80 #TODO Where to set this. This is percentage of all nodes in group that have mutation. Set at 80 because made sense to us.
# Eval 50% - 100%? 
scores_sig_all <- list()
...
saveRDS(scores_sig_all, file= here("4_Output_Data", "4_5_DENV", paste0("scores_sig_all_uw_", upstream_window,"_dw_", downstream_window, "_", st,".RData")))

# scores_all$Dengue_1_E_codon[[7]]
# head(scores_all$Dengue_1_C_codon[[7]])
# head(scores_all$Dengue_1_E_codon[[7]])
# 
# scores_sig_all$Dengue_1_E_codon
# scores_sig_all$Dengue_2_E_codon
# scores_sig_all$Dengue_1_C_codon
# scores_sig_all[["Dengue_2_whole_genome_codon"]]
# scores_sig_all[["Dengue_3_whole_genome_codon"]]
```
...
##Todo update with gene_df instead
...
plot_dengue_genome_AA_density <- function(scores, genes_table, gene_colors = NULL) {
  genes_table <- dplyr::filter(genes_table, tolower(gene) != "whole_genome")
...
# ########################################################################################################################################
# dengue_to_plot = 'Dengue_3'
# dengue_to_plot_num <- sub("Dengue_", "", dengue_to_plot)  # gives "2" for example
# gene_to_plot = 'NS5'
# plot_dataset_with_inferred_reconstruction = reconstructions[[paste0(dengue_to_plot,"_",gene_to_plot)]]$dataset_with_inferred_reconstruction_AA
...
# 
# 
# ## scores_world_sig_all_AA contains a list of singificant mutations for each of the phylowave-found groups (13 for SARS). 
# ## Each mutation entry takes the form "gene: AA_reference | AA_position | AA_variant" (eg. "N:D|3|L" or "ORF14:G|50|N") 
# 
# 
# plot_AA_mat_total = matrix(rep(100, length(which(plot_dataset_with_inferred_reconstruction$is.node == 'no'))), ncol = 1)
# ## Creates a matrix (AA_mat_total) initialized with the value 100 for each sequence that is not a node in the phylogenetic tree.
# ## This will be updated with mutation information later.
# 
# 
# ## Loops through predefined mutation groups, processing each one. This is the number of groups found by the index for DENV
# for(group_to_plot in seq_along(plot_score_list)){
#   ## Retrieves the list of significant group-defining amino acid mutations (scores_world_sig_all_AA) for the current group and current gene_to_plot
#   AAs = plot_score_list[[group_to_plot]]
#   ## AAs = AAs[which(unlist(lapply(AAs, function(x)str_split(x, ':')[[1]][1])) == gene_to_plot)]
#   ## Above line would process different score format: Only processes groups that contain at least one mutation and extracts the details stored in the string: "gene: AA_reference | AA_position | AA_variant"
#    if(length(AAs) >= 1){ 
#      print(paste("Processing group", group_to_plot, "with", length(AAs), "mutations"))
#      print(AAs)
...
#     ## For eg: AAs = "S:D|614|G" results in AAs_pos = 614, AAs_ref = D and AAs_var = G 
#     
#     AA_mat = matrix(plot_dataset_with_inferred_reconstruction[which(plot_dataset_with_inferred_reconstruction$is.node == 'no'),
#                                                          which(colnames(plot_dataset_with_inferred_reconstruction) == AAs_pos[1])], ncol = 1)
...
#     ## Updates the AA matrix (AA_mat) by marking each sequence that has this variant with the group number. 
#     ## The column named for the mutaiton/SNP will now read: 
#     ## 1. NA if the row/sequence has neither the reference nor variant amino acid. 
#     ## 2. The group_to_plot (eg 13) if the sequence has the VARIANT amino acid.
#     ## 3. 0 if the sequence has the REFERENCE amino acid.
#     
#     ## Continue with other group defining mutations in that group if they exisit 
#     if(length(AAs) > 1){
#       for(i in 2:length(AAs)){
#         AA_mat = cbind(AA_mat, plot_dataset_with_inferred_reconstruction[which(plot_dataset_with_inferred_reconstruction$is.node == 'no'),
#                                                                     which(colnames(plot_dataset_with_inferred_reconstruction) == AAs_pos[i])])
...
#       }
#     }
#     ## Adds a separator column with 100 to visually separate mutation groups
#     AA_mat = cbind(AA_mat,
#                    matrix(rep(100, length(which(plot_dataset_with_inferred_reconstruction$is.node == 'no'))), ncol = 1))
#     ## Assigns column names using the mutation group and mutation details. So "S:D|614|G" becomes "13:S:D|614|G"
#     colnames(AA_mat) = c(paste0(group_to_plot, ':', AAs), group_to_plot) 
#     ## Assigns row names using the sequence names.
#     rownames(AA_mat) = plot_dataset_with_inferred_reconstruction$name_seq[which(plot_dataset_with_inferred_reconstruction$is.node == 'no')]
#     
#     plot_AA_mat_total = cbind(plot_AA_mat_total, 
#                          AA_mat)
#     ## Appends the current group’s matrix (AA_mat) to the overall mutation matrix (AA_mat_total) so that the loop can continue
#   }
# }
# 
# 
...
# 
# 
# ## What is outputted is a Matrix whose rows are sequences/strains, and whose columns are significant group defining mutations 
# ## that are grouped by phylowave grouping. There are spacer columns of 100s between groups of "group" columns for readability and graphing. 
# ## If a strain carries that mutaiton/SNP the cell will have a number corresponding to the group associated with that mutaiton (eg 1-13)
# ## If a strain has the reference amino accid at that positon the cell will read 0 and NA if the strain has a diferrent AA from both the ref and variant
# 
# ## At this stage the matrix is essentially colored by group. 
# ## The 100s spacers now become grey vertical bars. And strains without the mutation at that position are white
# 
# 
# 
# plot_tree_Dengue_gene_groups <- ggtree(tree_to_plot, mrsd = lubridate::date_decimal(max(times_seqs_to_plot)), size = 0.08) %<+% node_data_to_plot +
...
# 
# 
# plot_tree_Dengue_gene_groups
# title_label <- paste0(dengue_to_plot, " | ", gene_to_plot)
# 
# ## Plot the AA_mat as a heatmap colored by group showing strains with the lineage definining SNPs
# plot_tree_Dengue_gene_groups_heat = gheatmap(plot_tree_Dengue_gene_groups, 
#                                        plot_AA_mat_total, 
#                                        offset=0.1, width=10, 
#                                        colnames=T, 
...
#   )
# 
# plot_tree_Dengue_gene_groups_heat
# 
# ggsave(filename = here("3_Output_Figures",paste0("Tree_",dengue_to_plot, "_defining_", gene_to_plot, "_mutations.pdf")),
#        plot = plot_tree_Dengue_gene_groups_heat, device = 'pdf', scale = 1, width = 25, height = 18, units = 'cm')
# ########################################################################################################################################


## Repeat for other genes of interest (vs S) with other reconstructions (vs reconstruction_world_S) or other locations (vs World) 
...
plot_dengue_gene_heatmap <- function(dengue_to_plot, gene_to_plot) {
  ...
  for (group_to_plot in seq_along(plot_score_list)) {
    ...
    if (length(AAs) >= 1) {
      ...
      colnames(AA_mat) <- paste0(group_to_plot, ':', AAs)
...
for(gene in genes_to_plot){
  cat("Plotting:" ,gene, "\n")
  try({
plot_dengue_gene_heatmap("Dengue_1", gene)
}, silent = TRUE )
}


plot_dengue_gene_heatmap("Dengue_3", "E")


save.image("06_30_25.RData")
