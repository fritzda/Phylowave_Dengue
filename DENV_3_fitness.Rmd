...
model_timeshift <- cmdstan_model(stan_file = '2_Functions/Model_multinomial_logistic_birthdeath_lineage_fitness_timeshift_20250617.stan')
...
res_fitness_3 = estimate_rel_fitness_groups_with_branches(dataset_with_nodes = dataset_with_nodes_3,
                                                        ...
                                                        t_breakpoint = NULL) #increased warmup and sampling from 250 and 500
...
breakpoints <- seq(1980, 2014, by = 0.5)
<<<<<<< HEAD
loo_scores_3 <- list()
=======
loo_scores <- list()
...
for (bp in breakpoints) {
  fit_result <- estimate_rel_fitness_groups_with_branches(
    dataset_with_nodes = dataset_with_nodes_3,
    tree = tree_DENV3,
    min_year = min_year,
    window = wind,
...
<<<<<<< HEAD
  loo_scores_3[[as.character(bp)]] <- loo(fit_result$fit$draws("log_lik"))
}

# Compare and visualize

loo_no_breakpoint_3 <- loo(res_fitness_3$fit$draws("log_lik"))
loo_scores_3[["no_breakpoint"]] <- loo_no_breakpoint_3

loo_comparison_3 <- loo::loo_compare(loo_scores)
print(loo_comparison_3)

# Convert loo_comparison to a data frame
loo_df_3 <- as.data.frame(loo_comparison_3)
loo_df_3$breakpoint <- rownames(loo_df_3)

# Assign numeric value for breakpoints, and manually set 2050 for "no_breakpoint"
loo_df_3 <- loo_df_3 %>%
  mutate(
...
# Find the breakpoint year of the best model
best_model_year_3 <- loo_df_3 %>%
  filter(elpd_diff == max(elpd_diff, na.rm = TRUE)) %>%
  pull(breakpoint_num)

# Plot
ggplot(loo_df_3, aes(x = breakpoint_num, y = elpd_diff)) +
  geom_point() +
  geom_vline(xintercept = best_model_year_3, linetype = "dashed", color = "blue") +
  annotate("text", x = best_model_year_3, y = max(loo_df$elpd_diff, na.rm = TRUE), 
           label = paste("Best model:", best_model_year_3), hjust = -0.1, vjust = -1, color = "blue") +
  ...
  annotate("text", x = 2030, y = max(loo_df_3$elpd_diff, na.rm = TRUE), label = "No Breakpoint", hjust = 1.1, vjust = 1, color = "red") +
  geom_hline(yintercept = 0, linetype = "dashed") +
  labs(
    x = "Breakpoint Year",
    y = "ELPD Difference vs. Best Model",
    title = "Model Fit Across Breakpoints (Higher is Better). DENV3"
  ) +
  theme_minimal()
=======
...
Before doing this make sure you have reconstruction files that have the AA and codon reconstructions in them

# Old Code for making reconstructions
Instead use 2_5_reconstruct_AA_changes

```{r reconstruction_build, eval=FALSE}

########################################################################################################################################
## Reconstruction
########################################################################################################################################
ORFs = c( "2Kfrag", "NS4B", "NS4A", "NS3",  "NS2B", "NS4A", "NS1", "prM", "E", "C", "NS5") 
#Can use mutiple orfs here if you have orf fasta files
...
orf_mapping <- list(
  ...
  "NS2A" =  "nonstructural protein NS2A underscore_bioconda.vcf",
  ...
  "NS5" = "nonstructural protein NS5 underscore_bioconda.vcf",
...
for (loc in reconstruction_locations) {
  for (orf in names(orf_mapping)) {
    file_path <- here("4_Output_Data", "4_5_DENV", paste0(loc, "_Thai_timetree_reconstruction_", orf,"04_02_25.rds"))
...
## Find significant snps
########################################################################################################################################
sig_threshold <- 0.75 #TODO Where to set this. This is percentage of all nodes in group that have mutation. Set at 80 because made sense to us.
# Eval 50% - 100%? 
scores_sig_all <- list()
...
# scores_sig_all[["Dengue_1_whole_genome_codon"]]
...
gene_colors <- MetBrewer::met.brewer(name = "Derain", n = nrow(genes))
...
plot_dengue_gene_heatmap <- function(dengue_to_plot, gene_to_plot) {
  ...
  ggsave(
    filename = here("3_Output_Figures", paste0("Tree_", dengue_to_plot, "_defining_", gene_to_plot, "_mutations.pdf")),
    plot = heatmap_plot,
    device = 'pdf',
    scale = 1,
    width = 25,
...
genes_to_plot <- c("C"  ,  "prM" , "E"   , "NS1"  ,"NS2A" ,"NS2B", "NS3" , "NS4A" ,"NS4B" ,"NS5", "whole_genome" )
...
plot_dengue_gene_heatmap("Dengue_1", "prM")



  
  ggsave(filename = here("3_Output_Figures", paste0("Tree_", dengue_to_plot, "_defining_", gene_to_plot, "_mutations.pdf")),
         plot = heatmap_plot, device = 'pdf', scale = 1, width = 25, height = 18, units = 'cm')
