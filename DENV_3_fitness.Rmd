...
run_single_combination <- function(upstream_window, downstream_window, sig_threshold,
  ...
  tryCatch({
    ...
    for (recon_name in names(reconstructions_local)) {
      ...
      # Validate expected structure: Dengue_1_NS3 → 3+ parts
      if (length(parts) < 3) {
...
results <- future_map2(
  .x = param_grid$upstream_window,
  .y = param_grid$downstream_window,
  .f = ~ run_single_combination(.x, .y, 0.8,
                                reconstructions, dataset_with_nodes_all, tree_DENV_all),
