...
#### Splitting genome vcfs into gene-specific vcfs ####

# Define the base directory for the VCFs
vcf_dir <- here("1_Data", "1_5_DENV", "Seq_vcf_data", "denv_seqs_wgs", "vcf_from_fasta")

# Mapping from serotype name to VCF filename
whole_genome_vcf_mapping <- list(
  "Dengue_1" = "d1.vcf",
  "Dengue_2" = "d2.vcf",
  "Dengue_3" = "d3.vcf",
  "Dengue_4" = "d4.vcf"
...
# Load and store each VCF into a named list
vcf_data <- list()

for (serotype in names(whole_genome_vcf_mapping)) {
  file_path <- here(vcf_dir, whole_genome_vcf_mapping[[serotype]])
  
  if (file.exists(file_path)) {
    vcf <- read.vcfR(file_path)
    fixed <- as.data.frame(vcf@fix)
    gt <- as.data.frame(vcf@gt)
    combined <- cbind(fixed, gt)
    
    vcf_data[[serotype]] <- combined
  } else {
