...
Several of the DENV 3 strains are missing clade asignments, these were
assigned by eye.
...
# Find DENGUE clades based on index dynamics: Potential Splits
...
Threshold node support: Max lineages is the same as number of lineages.
Idea is restrict search space to certain level. Trying to restrict space
to only well supported nodes. Each node has support value: 1.
bootstrap/posterior (similar to Tb tree) support comes from data Creates
sparsing issues in viruses that don't change much. 2. Proxy of how many
mutations ahead of branch. If two nodes are separated by a short amount
of time then the hypothesis that both nodes are distinct is less likely.
Uses branch lenght between nodes. Only permit nodes that are at least 1
mutation apart. Length must be how long it takes for 1 mutation.

Using time_window_initial + increment: sometimes helpful if wanting to
look at newer lineages. But elbow plots are harder to interpret as
depeneding on time params. Not recommended.

min_descendants_per_tested_node: only considering nodes with n tip and
node descendents. A filter or partition of the tree.

Index dynamics are made of many splines (each with a k), p values for
the spline need to better explain dynamics. k is the number of knots

Weight by time: Seqiences are a time series, this compensates for some
times where there are no sequences. Thus the model is trying to
overexplain the densely time sampled variants. Penalty for higher
samples in higher sampled times. Weight by time is the bucket/slice of
time in years. weighting_transformation: how the penalty is applied. Can
try all of these but can't really compare between compare since AIC and
BIC are poor measure of GAM comparison.
...
is easy to ground in math. Elbow plot analysis is basically arbitrary.
First elbow is the known dynamics, the second elbow is more exploratory.
Stopping at the first linear point on horizontal line.

Use knees to detect elbow. See `Knee Analysis.Rmd` for the linlog, AIC, BIC comparison plots. 
...
for (i in 1:4) {
  ...
  ## Add provenance params
  attr(split, "params") <- list(
    head_note = "Merged group splits with merging",
    created_utc = format(Sys.time(), tz = "UTC"),
    git_commit = Sys.getenv("GIT_COMMIT", unset = NA_character_),
    dengue = d,
    group_count_threshold = 30,
    group_freq_threshold = 0.01,
    metadata_dim = dim(dataset_with_nodes),
    metadata_columns = colnames(dataset_with_nodes),
    initial_splits_params = attr(potential_splits, "params")
  )
...
# Unmerged groups, all groups form potential splits included
for (i in 1:4) {
  ## Dynamically retrieve the relevant timed tree and dataset_with_nodes
  timed_tree <- get(paste0('tree_DENV', i))
  dataset_with_nodes <- get(paste0('dataset_with_nodes_', i))
  ...
  split <- merge.groups(
    ...
    metadata = dataset_with_nodes,
    initial_splits = potential_splits$potential_splits,
    group_count_threshold = 0,
    group_freq_threshold = 0.0000001
  )
  
  ## Add provenance params
  attr(split, "params") <- list(
    head_note = "Merged group splits with little to no merging",
    created_utc = format(Sys.time(), tz = "UTC"),
    git_commit = Sys.getenv("GIT_COMMIT", unset = NA_character_),
    dengue = d,
    group_count_threshold = 0,
  ...
  ## Dynamically assign the result to split_1, split_2, etc.
  assign(paste0('split_max_groups_', i), split)
...
created_utc = format(Sys.time(), tz = "UTC", "%Y-%m-%d_%H%M")
splits_merged <- get(glue("split_{d}"))
splits_unmerged <- get(glue("split_max_groups_{d}"))

splits_merged_file_name <- glue("4_Output_Data/merged_splits_DENV{d}_{created_utc}.rds")
splits_unmerged_file_name <- glue("4_Output_Data/unmerged_splits_DENV{d}_{created_utc}.rds")

write_rds(splits_merged, splits_merged_file_name)
write_rds(splits_unmerged, splits_unmerged_file_name)
...
mod <- potential_splits$best_mod[[k]]
mod_sub1 <- potential_splits$best_mod[[k-1]]

fits_list <- map(seq_len(k), function(j) {
  # fitted values from the j-th best model
  fv <- as.numeric(predict(potential_splits$best_mod[[j]]))  # using training data of that step
  fv <- exp(fv)
})

# back-transform if your fit used log_y = TRUE

pred_df <- dataset_with_nodes  %>%
  mutate(
    !!!setNames(fits_list, paste0("fitted_", seq_len(k)))
  )



# if you already have a hex color per tip in `group_color` ----
# (no legend; uses the exact hex per row)
ggplot(pred_df, aes(time, index, colour = group_color)) +
  geom_point(alpha = 0.8, size = 0.8) +
  # geom_point(aes(y = fitted, group = groups), alpha = 0.8, size = 0.8) +
  # geom_line(aes(y = fitted_1, group = groups), linewidth = 0.6, show.legend = FALSE) +
  # geom_line(aes(y = fitted_2, group = groups), linewidth = 0.6, show.legend = FALSE) +
  # geom_line(aes(y = fitted_3, group = groups), linewidth = 0.6, show.legend = FALSE) +
  # geom_line(aes(y = fitted_4, group = groups), linewidth = 0.6, show.legend = FALSE) +
  geom_line(aes(y = fitted_8, group = groups), linewidth = 0.6, show.legend = FALSE) +


  scale_colour_identity() +
  labs(
    title = sprintf("DENV%d: observed vs fitted (final step = %d)", i, k),
    x = "Time", y = "Index"
  ) +
  theme_minimal()
...
score (Vehtari and Ojanen, 2012), but we’re not aware of people doing
that and we don’t recommend it as PSIS-LOO has better diagnostics.
...
≪𝑁 , as then WAIC tends to have slightly smaller error, but in that case
both PSIS-LOO and WAIC have very small error and it doesn’t matter which
...
categories log probabilities would be log(1/22)≈−3.1 , and thus there
was two orders of magnitude error in log scale. With the fixed code
individual elpd_loo values were about −2.3\>−3.1 , that is, the model
...
## TODO breakpoints model takes too long on 8 core mac with lots of groups. Need more power
...
#Help to assess degree of improvement
